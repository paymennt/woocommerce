...
if (in_array('woocommerce/woocommerce.php', $active_plugins)) {
    ...
    add_filter('woocommerce_payment_gateways', 'add_paymennt_gateway');

    function add_paymennt_gateway($gateways)
    {
        $gateways[] = 'WC_Gateway_Paymennt';

        return $gateways;
    }

    add_action('plugins_loaded', 'init_paymennt_gateway');

    function init_paymennt_gateway()
    {
    ...
    function woocommerce_paymennt_actions()
    {
        if (isset($_GET['wc-api']) && !empty($_GET['wc-api'])) {
            WC()->payment_gateways();
            if ($_GET['wc-api'] == 'wc_gateway_paymennt_process_response') {
                do_action('woocommerce_wc_gateway_paymennt_process_response');
            }
        }
    }
